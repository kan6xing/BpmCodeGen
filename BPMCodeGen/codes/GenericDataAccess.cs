...
/// </summary>
public class GenericDataAccess:PubDataAccess
{
    ...
    private DbCommand CreateCommandAndAda()
    {
        ...
        dbada = factory.CreateDataAdapter();
        DbCommandBuilder dbBuild = factory.CreateCommandBuilder();
        dbBuild.DataAdapter = dbada;
        return comm;
    }

    public DataTable GetTableByada(string sqlSelectStr)
    { 
        DataTable dt=new DataTable();
        dcomm=CreateCommandAndAda();
        dcomm.CommandText=sqlSelectStr;
        dcomm.CommandType=CommandType.Text;
        dbada.SelectCommand=dcomm;
      
        dbada.Fill(dt);
        return dt;
    }

    public bool UpdateByada(DataTable dt)
    {
       
       return (dbada.Update(dt)>0);
    }
    ...
    public static DataTable Query(string sqlStr, params   SqlParameter[] cmdParms)
    {
        DbCommand comm = CreateCommand(sqlStr);
        foreach (SqlParameter sqlp in cmdParms)
        {
            DbParameter dbp = comm.CreateParameter();
            dbp.ParameterName = sqlp.ParameterName;
            dbp.Value = sqlp.Value;
            switch (sqlp.SqlDbType)
            {
...
//public class GenericAdapterDataAccess
//{
//    DbCommand dbcomm = GenericDataAccess.CreateCommand();
//    DbDataAdapter dba=
//}
