    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (searchType == "QuickSearch")
        {
            ...
            if (!String.IsNullOrEmpty(keyword))
                filter = queryProvider.CombinCond(filter, String.Format("toStr LIKE N'%{0}%' TelStr LIKE N'%{0}%' PhoneStr LIKE N'%{0}%' unit7Str LIKE N'%{0}%' or ", queryProvider.EncodeText(keyword)));
        }
        ...
        string query = String.Format("with T as (select top {0} ROW_NUMBER() OVER(order by {1}) as RowNum,* from ExpressTo {2}) select * from T where RowNum >= {3};select count(*) from ExpressTo {2}",
            gridPageInfo.RowNumEnd,
            order,
            filter,
            gridPageInfo.RowNumStart);
...
item.Attributes.Add("TelStr", Convert.ToString(reader["TelStr"]));
item.Attributes.Add("PhoneStr", Convert.ToString(reader["PhoneStr"]));
item.Attributes.Add("unit7Str", Convert.ToString(reader["unit7Str"]));
