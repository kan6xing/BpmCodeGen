    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (searchType == "QuickSearch")
        {
            ...
            if (!String.IsNullOrEmpty(keyword))
                filter = queryProvider.CombinCond(filter, String.Format("shipperStr LIKE N'%{0}%' shipperTelStr LIKE N'%{0}%' shipperPhoneStr LIKE N'%{0}%' shipperAdd7Str LIKE N'%{0}%' or ", queryProvider.EncodeText(keyword)));
        }
        ...
        string query = String.Format("with T as (select top {0} ROW_NUMBER() OVER(order by {1}) as RowNum,* from ExpressShipper {2}) select * from T where RowNum >= {3};select count(*) from ExpressShipper {2}",
            gridPageInfo.RowNumEnd,
            order,
            filter,
            gridPageInfo.RowNumStart);
...
item.Attributes.Add("shipperTelStr", Convert.ToString(reader["shipperTelStr"]));
item.Attributes.Add("shipperPhoneStr", Convert.ToString(reader["shipperPhoneStr"]));
item.Attributes.Add("shipperAdd7Str", Convert.ToString(reader["shipperAdd7Str"]));
